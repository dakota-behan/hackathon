...
const _v1Cap = new Vector3();
const _v2Cap = new Vector3();
const _v3Cap = new Vector3();
...
class Capsule {
	...
	lineLineMinimumPoints(line1, line2) {
		...
		const r = _v1Cap.copy(line1.end).sub(line1.start);
		const s = _v2Cap.copy(line2.end).sub(line2.start);
		const w = _v3Cap.copy(line2.start).sub(line1.start);
