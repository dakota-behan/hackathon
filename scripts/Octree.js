...
class Octree {
  ...
  triangleSphereIntersect(sphere, triangle) {
    ...
    if (triangle.containsPoint(sphere.center)) {
      return {
        ...
        depth: Math.abs(_planeOct.distanceToSphere(sphere)),
      };
    }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (d < r2) {
        return {
          normal: sphere.center.clone().sub(_v2Oct).normalize(),
          point: _v2Oct.clone(),
  ...
  capsuleIntersect(capsule) {
    ...
    if (hit) {
      const collisionVector = _capsuleOct
        .getCenter(new Vector3())
        .sub(capsule.getCenter(_v1Oct));
      const depth = collisionVector.length();
